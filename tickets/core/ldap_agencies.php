...
if($rparameters['ldap_type']==1) { 
	ldap_set_option($ldap, LDAP_OPT_PROTOCOL_VERSION, 3);
}elseif($rparameters['ldap_type']==0) {
...
if ($ldapbind) 
{
	...
	for ($i=0; $i < $cnt_group; $i++) 
	{
		...
		$LDAP_group_name=mb_convert_encoding($data[$i]['distinguishedname'][0], 'UTF-8', 'ISO-8859-1'); //$LDAP_group_name=utf8_encode($data[$i]['distinguishedname'][0]);
		$LDAP_group_name_qry=$data[$i]['distinguishedname'][0]; 
		$LDAP_group_name=str_replace ('','OE', $LDAP_group_name); //special char oe treatment
		$LDAP_group_samaccountname=mb_convert_encoding($data[$i]['samaccountname'][0], 'UTF-8', 'ISO-8859-1');  //$LDAP_group_samaccountname=utf8_encode($data[$i]['samaccountname'][0]);
		$LDAP_group_samaccountname=str_replace ('','OE', $LDAP_group_samaccountname); //special char oe treatment
		$LDAP_group_mail=mb_convert_encoding($data[$i]['mail'][0], 'UTF-8', 'ISO-8859-1'); //$LDAP_group_mail=utf8_encode($data[$i]['mail'][0]);
		...
		$GS_group=$qry->fetch();
		$qry->closeCursor();
		if(empty($GS_group)) {$GS_group=array();}
		if(empty($GS_group['id'])){ $GS_group['id']='';}
		if(empty($GS_group['name'])){$GS_group['name']='';}
		if(empty($GS_group['mail'])){$GS_group['mail']='';}
		if(empty($GS_group['ldap_guid'])){$GS_group['ldap_guid']='';}
		if(empty($GS_group['0'])){$GS_group['0']='';}
		...
		if($_GET['action']=='simul')
		{
			if(!$GS_group[0])
			{
				...
				echo '<i class="fa fa-plus-circle text-success"></i><font class="text-success"> '.T_("Création de l'agence").': <b>'.$LDAP_group_samaccountname.'</b> '.$LDAP_group_objectguid.' (<font size="1">'.T_("Raison: Le guid du groupe LDAP n'a pas été trouvé dans la liste des agences GestSup").'</font>)</font><br />';
			} else {
		...
		$data2 = array();
		$data2_temp = array();
		$filter2="(&(objectCategory=user)(memberof:1.2.840.113556.1.4.1941:=$LDAP_group_name_qry))";
		if($rparameters['debug']){echo "<br/><font size='1'>query find users ldap_search(ldap, $ldap_url, $filter2)</font><br />";}
		...
		for ($i2=0; $i2 < $cnt_users; $i2++) 
		{
			...
			if(!isset($data[$i2]['userprincipalname'][0])) $data[$i2]['userprincipalname'][0] = '';
			...
			if(!isset($data2[$i2]['objectsid'][0])) $data2[$i2]['objectsid'][0] = '';
			...
			$LDAP_user_guid=mb_convert_encoding($data2[$i2]['objectguid'][0], 'UTF-8', 'ISO-8859-1'); // $LDAP_user_guid=utf8_encode($data2[$i2]['objectguid'][0]);
			...
			$LDAP_user_sid=SIDtoString($data2[$i2]['objectsid'][0]);
			$LDAP_user_cn=mb_convert_encoding($data2[$i2]['cn'][0], 'UTF-8', 'ISO-8859-1'); //$LDAP_user_cn=utf8_encode($data2[$i2]['cn'][0]);
			$LDAP_user_samaccountname=mb_convert_encoding($data2[$i2]['samaccountname'][0], 'UTF-8', 'ISO-8859-1'); // $LDAP_user_samaccountname=utf8_encode($data2[$i2]['samaccountname'][0]);
			$LDAP_user_upn=mb_convert_encoding($data2[$i2]['userprincipalname'][0], 'UTF-8', 'ISO-8859-1'); //$LDAP_user_upn=utf8_encode($data2[$i2]['userprincipalname'][0]);
			$LDAP_user_givenname=mb_convert_encoding($data2[$i2]['givenname'][0], 'UTF-8', 'ISO-8859-1'); //$LDAP_user_givenname=utf8_encode($data2[$i2]['givenname'][0]);
			$LDAP_user_sn=mb_convert_encoding($data2[$i2]['sn'][0], 'UTF-8', 'ISO-8859-1'); //$LDAP_user_sn=utf8_encode($data2[$i2]['sn'][0]);
			...
			$LDAP_user_streetaddress=mb_convert_encoding($data2[$i2]['streetaddress'][0], 'UTF-8', 'ISO-8859-1'); //$LDAP_user_streetaddress=utf8_encode($data2[$i2]['streetaddress'][0]);
			$LDAP_user_postalcode=$data2[$i2]['postalcode'][0]; 
			$LDAP_user_l=mb_convert_encoding($data2[$i2]['l'][0], 'UTF-8', 'ISO-8859-1'); //$LDAP_user_l=utf8_encode($data2[$i2]['l'][0]);
			$LDAP_user_company=mb_convert_encoding($data2[$i2]['company'][0], 'UTF-8', 'ISO-8859-1'); //$LDAP_user_company=utf8_encode($data2[$i2]['company'][0]);
			$LDAP_user_fax=$data2[$i2]['facsimiletelephonenumber'][0]; 
			$LDAP_user_title=mb_convert_encoding($data2[$i2]['title'][0], 'UTF-8', 'ISO-8859-1');  //$LDAP_user_title=utf8_encode($data2[$i2]['title'][0]);

			if($rparameters['ldap_login_field']=='UserPrincipalName' && $LDAP_user_upn){$LDAP_login=$LDAP_user_upn;} else {$LDAP_login=$LDAP_user_samaccountname;}
			
			if($rparameters['debug']) {echo "<font size=\"1\"> [<b>$LDAP_login</b>] LDAP DATA: guid=$LDAP_user_guid cn=$LDAP_user_cn samaccountname=$LDAP_user_samaccountname givenname=$LDAP_user_givenname sn=$LDAP_user_sn uac=$LDAP_user_uac mail=$LDAP_user_mail telephonenumber=$LDAP_user_telephonenumber streetaddress=$LDAP_user_streetaddress postalcode=$LDAP_user_postalcode l=$LDAP_user_l company=$LDAP_user_company fax=$LDAP_user_fax title=$LDAP_user_title</font><br>";}
			$cnt_total_users++;
			...
			if ($GS_user)
			{
				...
				if ($_GET['action']=='simul')
				{	
					if($LDAP_login!=$GS_user['login']) {$user_update=T_("l'identifiant").',';}
					...
					$qry2->closeCursor();
					if(empty($GS_agency['id'])) {$GS_agency=array(); $GS_agency['id']='';}
					...
					if($user_update) {echo '<i class="fa fa-arrow-circle-up text-success"></i><font class="text-success"> '.T_("Mise à jour de l'utilisateur").': <b>'.$LDAP_login.'</b> (<font size="1">'.T_("Raison: Le guid LDAP est identique à celui de GestSup et une différence à été trouvé dans ").' '.$user_update.'</font>)</font><br />';}

				} elseif($_GET['action']=='run') {
					//update GS user informations
					if($LDAP_login!=$GS_user['login']) 
					{
						$qry=$db->prepare("UPDATE `tusers` SET `login`=:login WHERE `ldap_guid`=:ldap_guid");
						$qry->execute(array('login' => $LDAP_login,'ldap_guid' => $LDAP_user_guid));
					...
					if($LDAP_user_sid!=$GS_user['ldap_sid']) {
						$qry=$db->prepare("UPDATE `tusers` SET `ldap_sid`=:ldap_sid WHERE `ldap_guid`=:ldap_guid");
						$qry->execute(array('ldap_sid' => $LDAP_user_sid,'ldap_guid' => $LDAP_user_guid));
						$user_update.=T_("le SID");
					}
					...
					$GS_agency=$qry2->fetch();
					$qry2->closeCursor();
					if(empty($GS_agency['id'])) {$GS_agency['id']='';}
					if(empty($GS_agency['name'])) {$GS_agency['name']='';}
					...
					if($user_update) {
						echo '<i class="fa fa-arrow-circle-up text-success"></i><font class="text-success"> '.T_("Utilisateur").': <b>'.$LDAP_login.'</b> '.T_("mis à jour").' (<font size="1">'.T_("Raison: Le guid LDAP est identique à celui de GestSup et une différence à été trouvé dans ").' '.$user_update.'</font>)</font><br />';
						logit('ldap_agency','Update user informations from LDAP for user '.$LDAP_login.' ('.$user_update.')','0');
					}
				...
				if ($_GET['action']=='simul')
				{
					echo '<i class="fa fa-plus-circle text-success"></i><font class="text-success"> '.T_("Création de l'utilisateur").': <b>'.$LDAP_login.'</b> (<font size="1">'.T_("Raison: Le guid de l'utilisateur trouvé dans l'annuaire LDAP n'est pas présent dans GestSup").'</font>)</font><br />';
				}elseif ($_GET['action']=='run')
				{
					echo '<i class="fa fa-plus-circle text-success"></i><font class="text-success"> '.T_("Utilisateur").': <b>'.$LDAP_login.'</b> '.T_("crée").' (<font size="1">'.T_("Raison: Le guid de l'utilisateur trouvé dans l'annuaire LDAP n'est pas présent dans GestSup").'</font>)</font><br />';	
					//db insert
					$qry=$db->prepare("
					INSERT INTO `tusers` (`login`,`firstname`,`lastname`,`profile`,`mail`,`phone`,`address1`,`zip`,`city`,`fax`,`ldap_guid`,`ldap_sid`) 
					VALUES (:login,:firstname,:lastname,'2',:mail,:phone,:address1,:zip,:city,:fax,:ldap_guid,:ldap_sid)
					");
					$qry->execute(array(
						'login' => $LDAP_login,
						...
						'ldap_sid' => $LDAP_user_sid
						));
					logit('ldap_agency','Create user '.$LDAP_login,'0');
				}
			}	
		}
	...
	for ($i3=0; $i3 < $cnt_users; $i3++) 
	{
		...
		$LDAP_user_guid=mb_convert_encoding($data3[$i3]['objectguid'][0], 'UTF-8', 'ISO-8859-1'); //$LDAP_user_guid=utf8_encode($data3[$i3]['objectguid'][0]);
