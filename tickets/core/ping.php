...
if ($argv[1]=='globalping')
{
	if($argv[2]==$rparameters['server_private_key'] && $rparameters['server_private_key'])
	{
		...
		$qry=$db->prepare("SELECT tassets.id AS asset_id,tassets_iface.id AS asset_iface_id, tassets.netbios AS asset_netbios, tassets_iface.ip AS asset_iface_ip 
		FROM tassets_iface,tassets 
		WHERE 
		tassets.id=tassets_iface.asset_id AND 
		tassets_iface.ip!='' AND 
		tassets.state='2' AND 
		tassets.disable='0' AND 
		tassets_iface.disable='0' 
		ORDER BY tassets.id");
		$qry->execute();
			...
			while ($row=$qry->fetch())
			{
				...
				if(!$error)
				{
					...
					if((preg_match('#ms#', $result)))
					{
						...
						$qry2->execute(array('date_last_ping' => date("Y-m-d"), 'id' => $row['asset_id']));
						
						//update iface ping flag
						$qry2=$db->prepare("UPDATE `tassets_iface` SET `date_ping_ok`=:date_ping_ok WHERE `id`=:id");
						$qry2->execute(array('date_ping_ok' => date("Y-m-d H:i:s"), 'id' => $row['asset_iface_id']));
					} else {
						echo ' KO '.PHP_EOL;
						//update iface ping flag
						$qry2=$db->prepare("UPDATE `tassets_iface` SET `date_ping_ko`=:date_ping_ko WHERE `id`=:id");
						$qry2->execute(array('date_ping_ko' => date("Y-m-d H:i:s"), 'id' => $row['asset_iface_id']));
					}
			...
			if(!$error)
			{
				if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
					...
					if((preg_match('#Minimum#', $result)))
					{
						...
						//update asset flag
						$qry2=$db->prepare("UPDATE `tassets` SET `date_last_ping`=:date_last_ping WHERE `id`=:id");
						$qry2->execute(array('date_last_ping' => date("Y-m-d"),'id' => $globalrow['id']));
