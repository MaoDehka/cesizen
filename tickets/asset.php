...
$_POST['netbios']=htmlspecialchars($_POST['netbios'], ENT_QUOTES, 'UTF-8');
$_POST['ip']=htmlspecialchars($_POST['ip'], ENT_QUOTES, 'UTF-8');
$_POST['sn_internal']=htmlspecialchars($_POST['sn_internal'], ENT_QUOTES, 'UTF-8');
$_POST['description']=htmlspecialchars($_POST['description'], ENT_QUOTES, 'UTF-8');
$_POST['location']=htmlspecialchars($_POST['location'], ENT_QUOTES, 'UTF-8');
$_POST['date_stock']=htmlspecialchars($_POST['date_stock'], ENT_QUOTES, 'UTF-8');
...
if($_GET['id'] && ($_GET['action']!='new'))
{
	...
	$ripmodel=$qry->fetch();
	$qry->closeCursor();
	if(empty($ripmodel)) {$ripmodel=array();}
	...
	$ripmodel=$qry->fetch();
	$qry->closeCursor();
	if(empty($ripmodel)) {$ripmodel=array();$ripmodel['ip']='';}
	if($ripmodel['ip']=='1') {$ip_asset='1';} else {$ip_asset='0';}
}
...
if($rparameters['debug']) {echo "VAR: IP_ASSET=$ip_asset | WIFI_ASSET=$wifi_asset | globalrow[date_install]=$globalrow[date_install] POST[date_install]=$_POST[date_install]";}
...
if($_POST['date_stock']=='0000-00-00' || $_POST['date_stock']=='') {
	$_POST['date_stock']='';
} else {
	$_POST['date_stock'] = DateTime::createFromFormat('Y-m-d', $_POST['date_stock']);
	$_POST['date_stock']=$_POST['date_stock']->format('d/m/Y');
}
...
if($_POST['date_install']=='0000-00-00' || $_POST['date_install']=='') {
	$_POST['date_install']='';
} else {
	$_POST['date_install'] = DateTime::createFromFormat('Y-m-d', $_POST['date_install']);
	$_POST['date_install']=$_POST['date_install']->format('d/m/Y');
}
...
if($_POST['date_end_warranty']=='0000-00-00' || $_POST['date_end_warranty']=='') {
	$_POST['date_end_warranty']='';
} else {
	$_POST['date_end_warranty'] = DateTime::createFromFormat('Y-m-d', $_POST['date_end_warranty']);
	$_POST['date_end_warranty']=$_POST['date_end_warranty']->format('d/m/Y');
}
...
if($_POST['date_standbye']=='0000-00-00' || $_POST['date_standbye']=='') {
	$_POST['date_standbye']='';
} else {
	$_POST['date_standbye'] = DateTime::createFromFormat('Y-m-d', $_POST['date_standbye']);
	$_POST['date_standbye']=$_POST['date_standbye']->format('d/m/Y');
}
...
if($_POST['date_recycle']=='0000-00-00' || $_POST['date_recycle']=='') {
	$_POST['date_recycle']='';
} else {
	$_POST['date_recycle'] = DateTime::createFromFormat('Y-m-d', $_POST['date_recycle']);
	$_POST['date_recycle']=$_POST['date_recycle']->format('d/m/Y');
}
?>
<div>
	<div >
					...
					<h5 class="card-title">
						...
						<?php
    						...
    						if($_GET['action']=='new') {
								...
								$rsn=$qry->fetch();
								$qry->closeCursor();
								if(!isset($rsn['sn_internal'])) {$row=array(); $rsn['sn_internal']=''; $rsn['netbios']='';}
								if($mobile==0){echo T_("Édition de l'équipement").' n°'.$rsn['sn_internal'].' : '.$rsn['netbios'];} 
								else {echo 'n°'.$rsn['sn_internal'].': '.$rsn['netbios'];}
							}
						?>
					</h5>
				...
				<div class="card-body p-0">
					<div class="p-3">
						<div class="row">
							<div class="col-lg-9">
								...
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<input style="min-width:269px;" class="form-control <?php if($mobile){echo 'col-7';} else {echo 'col-3';} ?>" name="sn_internal" id="sn_internal" type="text" size="25"  value="<?php if($_POST['sn_internal']) echo $_POST['sn_internal']; else echo $globalrow['sn_internal']; ?>"  />
									</div>
								</div>
								...
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<select style="min-width:269px;" class="form-control col-3 d-inline-block" id="type" name="type" onchange="submit();" >
											<?php
												...
												$qry->closeCursor();
												if(empty($globalrow['type']) && empty($_POST['type'])) {echo '<option value="" selected></option>';}
											?>
										</select>
										<select class="form-control col-3 d-inline-block" id="manufacturer" name="manufacturer" onchange="submit();" >
											<?php
												...
												while($row=$qry->fetch()) 
												{
													...
													{
														//check incorrect manufacturer value #4101
														if($globalrow['model'])
														{
															$qry2=$db->prepare("SELECT `manufacturer` FROM `tassets_model` WHERE id=:id");
															$qry2->execute(array('id' => $globalrow['model']));
															$row2=$qry2->fetch();
															$qry2->closeCursor();
															if(!isset($row2['manufacturer'])) {$row2=array(); $row2['manufacturer']='';}
															
															if($globalrow['manufacturer']!=$row2['manufacturer']){$globalrow['manufacturer']=$row2['manufacturer'];}
														}

														if($globalrow['manufacturer']==$row['id']) {
										...
										<?php
											...
											$qry->closeCursor();
											if(empty($globalrow['model']) && empty($_POST['model'])) {echo '<option value="" selected></option>';}
										?>
										</select>
									</div>
								</div>
								...
								if($rright['asset_virtualization_disp']!=0)
								{
									...
									$row=$qry->fetch();
									$qry->closeCursor();
									if(empty($row['virtualization'])) {$row=array(); $row['virtualization']='';}
									if($row['virtualization'])
									{
										...
										<div class="form-group row">
											...
											<div class="col-sm-9">
												<label>
													<input name="virtualization" id="virtualization" type="checkbox" ';if($globalrow['virtualization']==1) {echo "checked";} echo ' class="ace mt-2" value="1">
													<span class="lbl"></span>
												</label>
											</div>
										</div>
								...
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<input style="min-width:269px;" class="form-control <?php if($mobile){echo 'col-7';} else {echo 'col-3';} ?>" name="netbios" id="netbios" type="text" size="25"  value="<?php if($_POST['netbios']) echo $_POST['netbios']; else echo $globalrow['netbios']; ?>"  />
									</div>
								</div>
								<!-- END netbios part -->
								...
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<select style="width:269px;" class="" id="user" name="user">
											<?php
											...
											if($rright['asset_list_company_only'])
											{
												...
												$qry=$db->prepare("SELECT `id`,`lastname`,`firstname`,`disable` FROM `tusers` ORDER BY id!=0, lastname ASC, firstname ASC ");
												$qry->execute();
											}

											//display user list
											while($row=$qry->fetch()) {
												if($row['id']=='0') {$row['id']=''; $row['lastname']='';}
												if($_POST['user']==$row['id']){
													echo '<option selected value="'.$row['id'].'">'.$row['lastname'].' '.$row['firstname'].'</option>';
												} if($globalrow['user']==$row['id']){
													echo '<option selected value="'.$row['id'].'">'.$row['lastname'].' '.$row['firstname'].'</option>';
												} elseif(!$row['disable']) {
								...
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<select style="min-width:269px;" class="form-control <?php if($mobile){echo 'col-7';} else {echo 'col-3';} ?>" id="department" name="department" >
											<?php
									...
									if($rparameters['asset_ip'])
									{
										...
										if($iface_counter[0]>0)
										{
											...
											while($row=$qry->fetch()) 
											{
												...
												<div class="form-group row">
													<div class="col-sm-2 col-form-label text-sm-right pr-0">
														<label class="mb-0" for="netbios_'.$row['id'].'">
															';
															//display ping flags
													...
													<div class="col-sm-9">
														<input style="min-width:269px;" class="form-control col-3 d-inline-block" name="netbios_'.$row['id'].'" id="netbios_'.$row['id'].'" type="text" placeholder="Nom NetBIOS" size="12" value="';if($_POST["netbios_$row[id]"]) {echo $_POST["netbios_$row[id]"];} else { echo $row['netbios'];} echo'" />
								...
								if($globalrow['virtualization']==0)
								{
									...
									<div class="form-group row">
										...
										<div class="col-sm-9">
											<input style="min-width:269px;" class="form-control '; if($mobile){echo 'col-7';} else {echo 'col-3';} echo '" name="sn_manufacturer" id="sn_manufacturer" type="text" value="'; if($_POST['sn_manufacturer']) echo $_POST['sn_manufacturer']; else echo $globalrow['sn_manufacturer']; echo '"  />
										</div>
									</div>
									';
								}
								...
								if($globalrow['virtualization']==0)
								{
									...
									<div class="form-group row">
										...
										<div class="col-sm-9">
											<input style="min-width:269px;" class="form-control  '; if($mobile){echo 'col-7';} else {echo 'col-3';} echo '" name="sn_indent" id="sn_indent" type="text" value="'; if($_POST['sn_indent']) {echo $_POST['sn_indent'];} else {echo $globalrow['sn_indent'];} echo '"  />
										</div>
									</div>
									';
								}
								...
								if($rright['asset_location_disp']!=0 && $globalrow['virtualization']==0)
								{
									...
									<div class="form-group row">
										...
										<div class="col-sm-9">
											<select style="width:269px;" id="location" name="location">
												';
								...
								if($ip_asset=='1' && $globalrow['virtualization']==0 && $rparameters['asset_ip'])
									{
										...
										<div class="form-group row">
											...
											<div class="col-sm-9">
												<input style="min-width:269px;" class="form-control '; if($mobile){echo 'col-7';} else {echo 'col-3';} echo '" name="socket" id="socket" type="text" size="25"  value="'; if($_POST['socket']) echo $_POST['socket']; else echo $globalrow['socket']; echo '"  />
											</div>
										</div>
										';
									}
								...
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<select style="min-width:269px;" class="form-control <?php if($mobile){echo 'col-7';} else {echo 'col-3';} ?>" id="technician" name="technician">
											<?php
											...
											$row=$qry->fetch();
											$qry->closeCursor();
											if(empty($row['lastname'])) {$row=array(); $row['lastname']='';}
								...
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<select style="min-width:269px;" class="form-control <?php if($mobile){echo 'col-7';} else {echo 'col-3';} ?>" id="maintenance" name="maintenance">
											<?php
								...
								if($globalrow['virtualization']==0)
								{
									...
									<div class="form-group row">
										...
										<div class="col-sm-9">
											<input style="min-width:269px;" class="form-control datetimepicker-input '; if($mobile){echo 'col-7';} else {echo 'col-3';} echo '" autocomplete="off" data-toggle="datetimepicker" data-target="#date_stock" type="text" size="25" name="date_stock" id="date_stock" value="'; if($_POST['date_stock']) echo $_POST['date_stock']; else echo $globalrow['date_stock']; echo '" >
										</div> 
									</div>
									';
								}
								...
								<?php
									echo '
										<div class="form-group row">
											...
											<div class="col-sm-9">
												<input style="min-width:269px;" class="form-control datetimepicker-input '; if($mobile){echo 'col-7';} else {echo 'col-3';} echo '" data-toggle="datetimepicker" data-target="#date_install" autocomplete="off" type="text" size="25" name="date_install" id="date_install" value="'; if($_POST['date_install']) echo $_POST['date_install']; else echo $globalrow['date_install']; echo'" >
											</div> 
										</div>
									';
								?>
									...
									if($globalrow['virtualization']==0)
									{
										...
										echo '
											<div class="form-group row">
												...
												<div class="col-sm-9">
													<input style="min-width:269px;" class="form-control datetimepicker-input '; if($mobile){echo 'col-7';} else {echo 'col-3';} echo '" data-toggle="datetimepicker" data-target="#date_end_warranty"  autocomplete="off" type="text" size="25" name="date_end_warranty" id="date_end_warranty" value="'; if($_POST['date_end_warranty']) echo $_POST['date_end_warranty']; else echo $globalrow['date_end_warranty']; echo'" >
												</div> 
											</div>
										';
									}
									...
									if($_GET['action']!='new' && $globalrow['state']!='2' && $globalrow['state']!='1' )
									{
										echo '
											<div class="form-group row">
												...
												<div class="col-sm-9">
													<input style="min-width:269px;" class="form-control datetimepicker-input '; if($mobile){echo 'col-7';} else {echo 'col-3';} echo '" data-toggle="datetimepicker" data-target="#date_standbye" type="text" size="25" name="date_standbye" id="date_standbye" value="'; if($_POST['date_standbye']) echo $_POST['date_standbye']; else echo $globalrow['date_standbye']; echo '" >
												</div> 
											</div>
										';
									}
								...
								if($globalrow['state']=='4' || $_POST['state']=='4')
								{
									echo '
										<div class="form-group row">
											...
											<div class="col-sm-9">
												<input style="min-width:269px;" class="form-control datetimepicker-input '; if($mobile){echo 'col-7';} else {echo 'col-3';} echo '" data-toggle="datetimepicker" data-target="#date_recycle" type="text" size="25" name="date_recycle" id="date_recycle" value="'; if($_POST['date_recycle']) echo $_POST['date_recycle']; else echo $globalrow['date_recycle']; echo '" >
											</div> 
										</div>
									';
								}
								...
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<select style="min-width:269px;" class="form-control <?php if($mobile){echo 'col-7';} else {echo 'col-3';} ?>" id="state" name="state">
											<?php
											//display states list
							...
							<div class="col-lg-3" >
								...
								<?php 
									...
									} elseif($_POST['type']!='') 
									{
										...
										$row=$qry->fetch();
										$qry->closeCursor();
										if(empty($row['image'])) {$row=array(); $row['image']='';}
										$model=$_POST['model'];
									} else {
										$qry=$db->prepare("SELECT `image` FROM `tassets_model` WHERE id LIKE :id");
									...
									if($row['image']!='') 
									{
										...
										} elseif(file_exists("./upload/asset/model/$row[image]"))
										{
											$img_size = getimagesize("./upload/asset/model/$row[image]");
											$img_width=$img_size[0];
											if($img_width>250) {$img_width='width="250"';} else {$img_width='';}
											echo '<img align="left" border="1" alt="image du modèle" '.$img_width.' src="./upload/asset/model/'.$row['image'].'" />';
										} 
									}
						...
						<div class="border-t-1 brc-secondary-l1 bgc-secondary-l3 py-3 text-center" >	
							<button title="CTRL+S" accesskey="s" name="modify" id="modify" value="modify" type="submit" class="btn btn-success">
