...
if($rparameters['debug']) {echo '<u><b>DEBUG MODE :</b></u><br /><b>VAR:</b> where_service='.$where_service.' | where_agency='.$where_agency.' |  POST_tech='.$_POST['tech'].' | POST_service='.$_POST['service'].' | POST_agency='.$_POST['agency'].' | POST_state='.$_POST['state'].' | cnt_agency='.$cnt_agency.' | cnt_service='.$cnt_service;}
?>
...
<form method="post" action="" name="filter" >
	...
	{
		echo '
			<select style="width:160px;" class="form-control form-control-sm d-inline-block" name="company" id="company" onchange="submit()" autocomplete="off">
				';
				$qry=$db->prepare("SELECT `id`,`name` FROM `tcompany` WHERE disable='0' ORDER BY name");
				$qry->execute();
				while($row=$qry->fetch()) 
				{
					if($row['id']==$_POST['company']) {
						echo '<option value="'.$row['id'].'" selected >'.$row['name'].'</option>'; 
					} else {
						echo '<option value="'.$row['id'].'" >'.$row['name'].'</option>'; 
					}
				...
				if ($_POST['company']=="%") {
					echo '<option value="%" selected>'.T_('Toutes les sociétés').'</option>';
				} else {
					echo '<option value="%" >'.T_('Toutes les sociétés').'</option>';
				}											
			echo '
			</select>
		';
	...
	?>
	<select style="width:160px;" class="form-control form-control-sm d-inline-block" name="tech" id="tech" onchange="submit()">
		<?php
			...
			$qry=$db->prepare("SELECT `id`,`name` FROM `tgroups` WHERE `type`=1 AND `disable`=0");
			$qry->execute();
			while($row=$qry->fetch()) 
			{
				if($_POST['tech']=='G_'.$row['id']) {
					echo '<option selected value="'.$row['id'].'">[G] '.$row['name'].'</option>'; 
				} else {
...
<?php
	...
	//case filter by technician group
	if(preg_match('/G_/',$_POST['tech']))
	{
		//get technician group id
		$tech_group_id=explode('G_',$_POST['tech']);
		...
		while($row=$qry->fetch()) 
		{
			$where_tech_group.="tincidents.technician=$row[user] OR ";
		}
		$qry->closeCursor();
		$where_tech_group=substr($where_tech_group, 0, -4);
		$where_tech_group.=')';
		/*
		echo 'tech_group'.$tech_group_id; 
		echo 'where_tech_group'.$where_tech_group; exit;
		*/
	...
	if($company_filter && $rparameters['user_advanced'])
	{
		...
		echo '<br />';
		echo '<a name="chart9"></a>';
		require('stats/pie_time_company.php');
		echo '<br />';
	}
