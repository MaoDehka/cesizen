...
# @Description : send asset of selected model
# @Call : ./ticket.php
# @Parameters :  
# @Author : Flox
# @Create : 24/10/2023
...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') 
{
    ...
    $qry=$db->prepare("SELECT `token`,`user_id`,`procedure_id` FROM ttoken WHERE action='ticket_access' AND `token`=:token AND `ip`=:ip");
    ...
    if(!empty($token['token'])) {
        ...
        $qry=$db->prepare("SELECT `id`,`netbios`,`user` FROM `tassets` WHERE `model` LIKE :asset_model ORDER BY `netbios`");
        $qry->execute(array('asset_model' => $_POST['ModelId']));
