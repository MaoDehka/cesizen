...
)
{
				...
				<div class="p-3">
					<form id="form" name="form" method="post"  action="">
						<fieldset>
							...
							<input style="width:auto;" class="form-control form-control-sm d-inline-block" name="name" id="name" type="text" value="'; if($rgroup['name']) echo "$rgroup[name]"; echo'" autocomplete="off" />
						</fieldset>
						<div class="pt-2"></div>
						';
						...
						<div class="radio">
							<label>
								...
								<span class="lbl"> '.T_("Groupe de demandeurs").'</span>
							</label>
						</div>
						<div class="radio">
							<label>
						...
						<fieldset>
							...
							<select style="width:auto;" class="form-control form-control-sm d-inline-block" name="user" id="user" >
								<option value=""></option>';
				...
				<div class="p-3">
					<form id="form" name="form" method="post"  action="">
						<div class="pl-4">
							<fieldset>
								<label for="name">'.T_('Nom').' :</label>
								<input style="width:auto;" class="form-control form-control-sm d-inline-block" name="name" id="name" type="text" value="" autocomplete="off" />
							</fieldset>
							<div class="pt-2"></div>
							';
							...
							if($cnt_service<=1 && $rright['admin']==0) //not show select field, if there are only one service, send data in background
								{
									...
									echo '
										<fieldset>
											<label for="service">'.T_('Service').' :</label>
											<select style="width:auto;" class="form-control form-control-sm d-inline-block" name="service" >
											';
												if($rright['dashboard_service_only']!=0 && $rparameters['user_limit_service']==1) {
													//display only service associated with this user
													$qry2 = $db->prepare("SELECT tservices.id,tservices.name FROM `tservices`,`tusers_services` WHERE tservices.id=tusers_services.service_id AND tusers_services.user_id=:user_id AND tservices.disable='0' ORDER BY tservices.name");
												...
												while ($row2=$qry2->fetch()) 
												{
													if(empty($row['service'])) {$row=array(); $row['service']='';}
													if(empty($row2['id'])) {$row2=array(); $row2['id']='';}
							...
							<div class="radio">
								<label>
									...
									<span class="lbl"> '.T_("Groupe de demandeur").'</span>
								</label>
							</div>
							<div class="radio">
								<label>
						...
						<tbody>';
							...
							while ($rgroup=$qry->fetch()) 
							{
								...
								<tr>
									...
									echo '
									<td>
										<a class="btn btn-sm btn-warning text-center" href="./index.php?page=admin&amp;subpage=group&amp;action=edit&amp;id='.$rgroup['id'].'"  title="'.T_('Modifier ce groupe').'" ><i style="color:#FFF;" class="fa fa-pencil-alt "><!----></i></a>
										<a class="btn btn-sm btn-danger text-center" onClick="javascript: return confirm(\''.T_('Êtes-vous sur de vouloir supprimer ce groupe ?').'\');" href="./index.php?page=admin&amp;subpage=group&amp;id='.$rgroup['id'].'&amp;type='.$_GET['type'].'&amp;action=delete"  title="'.T_('Supprimer ce groupe').'" ><i class="fa fa-trash"><!----></i></a>
									</td>
								</tr>';
							}
							echo '
	...
	echo '<div class="alert alert-danger"><strong><i class="fa fa-remove"><!----></i>'.T_('Erreur').':</strong> '.T_("Vous n'avez pas le droit d'accès à ce menu, ou vous ne disposer d'aucun service associé, contactez votre administrateur").'.<br></div>';
}
?>