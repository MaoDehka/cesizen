							...
							<div class="col-lg-9">
								...
								<div class="form-group row">
									...
									<div class="col-sm-9">
										...
										<?php
											if ($_POST['type'] && $_POST['manufacturer'])
											{
												$qry=$db->prepare("SELECT `id`,`name` FROM `tassets_model` WHERE `type` LIKE :type AND `manufacturer` LIKE :manufacturer ORDER BY `name` ASC");
												$qry->execute(array('type' => $_POST['type'],'manufacturer' => $_POST['manufacturer']));
												while($row=$qry->fetch()) 
												{
													if ($_POST['model'])
													{
