...
//default values
$defaultprofile=2; //1 is poweruser, 2 is user 
...
if($rparameters['user_register'])
{
	...
	if($_GET['token'])
	{
		...
		$qry=$db->prepare("SELECT `id`,`user_id` FROM `ttoken` WHERE `token`=:token AND `ip`=:ip");
		...
		if(isset($token['user_id']))
		{
			...
			$msg_success=T_("Votre compte à été activé, vous pouvez-vous connecter à l'application en cliquant sur le bouton Retour");
		} else {
			$msg_error=T_("Jeton invalide, merci de contactez votre administrateur");
		}
	}
    //actions on submit
	if($_POST['submit'])
        	        ...
        	        if($_POST['password']) {
        	             if($_POST['password2']) {
            	             if($_POST['mail']) {
										...
										if(!$msg_error && !$msg_success)
										{
											...
											if($rparameters['user_advanced'] && $_POST['company']) //find company if exist else create it
											{
												...
												if(empty($company['id']))
												{
													...
													$qry->execute(array('name' => $_POST['company']));
													if(!isset($company['id'])) {$company=array(); $company['id']=0;}
											...
											$qry=$db->prepare("INSERT INTO `ttoken` (`date`,`token`,`action`,`user_id`,`ip`) VALUES (NOW(),:token,'subscription',:user_id,:ip)");
											$qry->execute(array('token' => $token,'user_id' => $user_id,'ip' => $_SERVER['REMOTE_ADDR']));
            	                ...
            	                } else {$msg_error=T_("Vos mots de passe ne sont pas identiques");}
											...
											<div class="active show px-3 px-lg-0 pb-0" id="id-tab-login">
												...
												<form id="conn" method="post" action="" class="form-row mt-4"> 
													...
													<div class="form-group col-md-6 offset-md-3">
														<button type="submit" name="submit" id="submit" value="submit" class="btn btn-success btn-block btn-md btn-bold mt-2 mb-1">
