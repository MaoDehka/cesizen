...
if($_POST['validation'] && $ticket_id)
{
    ...
    if(!$_POST['user_validation'])
    {
        ...
        $qry=$db->prepare("UPDATE `tincidents` SET `state`='2', date_res='', user_validation_date='', user_validation='0', techread='0' WHERE `id`=:id");
        $qry->execute(array('id' => $ticket_id));
...
<html lang="fr">
							...
							<div class="p-3">
								<?php 
									if($rparameters['user_validation'])
									{
															...
															<div class="step-content row-fluid position-relative" id="step-container">
                                                                ...
                                                                <div class="col-xs-6 col-sm-10" id="content">
                                                                   <h4 class="lighter text-success pb-3 pt-3">'.T_('Votre ticket est-il bien résolu ?').'</h4>
                                                                    <div class="pl-4">
                                                                        <div class="radio">
                                                                            <label>
                                                                                <input name="user_validation" OnChange="UserValidation();" value="1" type="radio" checked class="ace">
                                                                                <span class="lbl"> '.T_('Oui').'</span>
                                                                            </label>
                                                                        </div>
                                                                        <div class="radio">
                                                                            <label>
                                                                                <input name="user_validation" OnChange="UserNoValidation();" value="0" type="radio" class="ace">
																...
																<div class="row-fluid wizard-actions text-center">
																	<button type="submit" id="validation" name="validation" value="validation" class="btn btn-success">
																		...
																		<span id="button">'.T_('Confirmer la résolution').'</span>
																	</button>
																</div>
															</div>
														</form>
										...
										echo DisplayMessage('error',T_("Le paramètre de validation demandeur n'est pas activé, contactez votre administrateur"));
									} 
									?>
							</div>
						</div><!-- /.card-body -->
    ...
    <!-- modify reason and button name if change radio buttons -->
    <script>                              
		function UserValidation() {
			$("#reason").addClass("d-none");
			document.getElementById('button').innerHTML='<?php echo T_('Confirmer la résolution'); ?>';
		}
		function UserNoValidation() {
			$("#reason").removeClass("d-none");
			document.getElementById('button').innerHTML='<?php echo T_('Rouvrir'); ?>';
		}
    </script>
</html>