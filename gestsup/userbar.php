...
//limit ticket to agency and service
if($rright['dashboard_agency_only'] && $rright['dashboard_service_only'] && $cnt_agency && $cnt_service)
{
	$where_agency=str_replace(')','',$where_agency);
	$where_service=str_replace('AND (','OR ',$where_service);
	if($cnt_agency==1) {$where_agency=str_replace('AND', 'AND (',$where_agency);}
	if(!strpos($where_service,')')) {$where_service=$where_service.')';} //case 1 service 2 agencies
}
...
#6666 techread remove for super
if($_SESSION['profile_id']==0 || $_SESSION['profile_id']==4)
{
	...
	$cnt3=$query->fetch();
	$query->closeCursor(); 
} else {$cnt3=array(); $cnt3[0]=0;}

$query=$db->query("SELECT COUNT(`id`) FROM `tincidents` WHERE $profile='$_SESSION[user_id]' $where_service_your $where_agency_your AND `userread`='0' AND `disable`='0'");
$cnt_user_unread=$query->fetch();
$query->closeCursor(); 
...
$query=$db->query("SELECT SUM(`tincidents`.`time_hope`-`tincidents`.`time`) FROM `tincidents`,`tstates` WHERE `tincidents`.`state`=`tstates`.`id` AND `tincidents`.`time_hope`-`tincidents`.`time`>0 AND `tincidents`.`technician` LIKE '$uid' AND `tincidents`.`disable`='0' AND `tstates`.`meta`='1' $where_agency $where_service $parenthese2");
$nbtps=$query->fetch();
$query->closeCursor();
...
if($rright['userbar']) //extended informations
{
	...
	<li class="nav-item dropdown dropdown-mega">
		<a title="'.$label_meta.'" class="nav-link dropdown-toggle pl-lg-3 pr-lg-4 purple" href="'.$link_meta_state.'" role="button" aria-haspopup="true" aria-expanded="false">
			...
			'; 
			if(!$mobile) {echo T_('À traiter');}
			echo '
			<span id="badge_todo" class="badge badge-sm badge-secondary radius-round text-80 border-1 brc-white-tp5">'.$nbatt2[0].'</span>
		</a>
	</li>
	';
	...
	if($rright['ticket_time_disp']!=0 && $rright['ticket_time_hope_disp']!=0)
	{
		...
		<li class="nav-item dropdown dropdown-mega">
			<a title="'.T_("Nombre d'heures de travail estimé dans vos tickets ouverts").'" class="nav-link dropdown-toggle pl-lg-3 pr-lg-4 purple" href="index.php?page=dashboard&amp;userid='.$_SESSION['user_id'].'&amp;state=1" role="button" aria-haspopup="true" aria-expanded="false">
				...
				echo '
				<span id="badge_load" class="badge badge-sm badge-info radius-round text-80 border-1 brc-white-tp5">'.$nbtps[0].'h</span>
			</a>
		</li>
		';
	}
} elseif($cnt_user_unread[0]) { //display unread for user
	echo'
	<li class="nav-item dropdown dropdown-mega">
		<a title="'.T_('Tickets en attente de lecture').'" class="nav-link dropdown-toggle pl-lg-3 pr-lg-4" href="index.php?page=dashboard&amp;userid='.$_SESSION['user_id'].'&amp;userread=0" role="button" aria-haspopup="true" aria-expanded="false">
			<i class="fa fa-bookmark text-110 icon-animated-bell mr-lg-2"><!----></i>
			<span id="badge_unread" class="badge badge-sm badge-warning radius-round text-80 border-1 brc-white-tp5">'.$cnt_user_unread[0].'</span>
		</a>
	</li>
	';
}
...
if($rparameters['user_limit_ticket'] && $ruser['limit_ticket_number'] && $ruser['limit_ticket_days'] && $ruser['limit_ticket_date_start']!='0000-00-00' &&($_SESSION['profile_id']==1 || $_SESSION['profile_id']==2))
{
	...
	<li class="nav-item dropdown dropdown-mega">
		<a title="'.T_('Nombre de tickets restants disponibles, valable du').' '.$date_start.' '.T_('au').' '.$date_end.'" class="nav-link dropdown-toggle pl-lg-3 pr-lg-4 purple" href="index.php?page=dashboard&amp;userid='.$_SESSION['user_id'].'&amp;state=3" role="button" aria-haspopup="true" aria-expanded="false">
			<i class="fa fa-ticket text-110 mr-lg-2"><!----></i>
			'; if(!$mobile) {echo T_('Tickets disponibles');} echo '
			<span id="badge_available_tickets" class="badge badge-sm badge-warning radius-round text-80 border-1 brc-white-tp5">'.$nbticketremaining.'</span>
		</a>
	</li>
	';
}
...
if($rparameters['company_limit_ticket'] && ($_SESSION['profile_id']==1 || $_SESSION['profile_id']==2))
{
	...
	if($rcompany['limit_ticket_number'] && $rcompany['limit_ticket_days'] && $rcompany['limit_ticket_date_start']!='0000-00-00' )
	{
		...
		<a title="'.T_('Nombre de tickets restants disponibles, valable du').' '.$date_start.' '.T_('au').' '.$date_end.'" class="nav-link dropdown-toggle pl-lg-3 pr-lg-4 purple" href="index.php?page=dashboard&amp;userid='.$_SESSION['user_id'].'&amp;state=3" role="button" aria-haspopup="true" aria-expanded="false">
			<i class="fa fa-ticket text-110 mr-lg-2"><!----></i>
			'; if(!$mobile) {echo T_('Tickets disponibles');} echo '
			<span id="id-badge_remaining_tickets" class="badge badge-sm badge-warning radius-round text-80 border-1 brc-white-tp5">'.$nbticketremaining.'</span>
		</a>
	</li>
		';
	}
...
if($rparameters['company_limit_hour'] && ($_SESSION['profile_id']==1 || $_SESSION['profile_id']==2))
{
	...
	if($rcompany['limit_hour_number'] && $rcompany['limit_hour_days'] && $rcompany['limit_hour_date_start']!='0000-00-00' )
	{
		...
		<a title="'.T_("Nombre d'heures restantes disponibles, valable du").' '.$date_start.' '.T_('au').' '.$date_end.'" class="nav-link dropdown-toggle pl-lg-3 pr-lg-4 purple" href="index.php?page=dashboard&amp;userid='.$_SESSION['user_id'].'&amp;state=3" role="button" aria-haspopup="true" aria-expanded="false">
			...
			<span id="badge_remain_hours" class="badge badge-sm badge-warning radius-round text-80 border-1 brc-white-tp5">'.$nbhourremaining.'h</span>
		</a>
	</li>
		';
	}
...
if($rright['ticket_billable']){
	$qry=$db->prepare("SELECT COUNT(`id`) AS `counter` FROM `tincidents` WHERE `billable`='1' AND `disable`='0' AND `technician`=:technician AND `state` IN (SELECT `id` FROM `tstates` WHERE `meta`='1')");
	$qry->execute(array('technician' => $_SESSION['user_id']));
	$billable_counter=$qry->fetch();
	$qry->closeCursor();
	echo '
		<li class="nav-item dropdown dropdown-mega">
			<a title="'.T_("Nombre de tickets marqués comme facturables, sur lesquels vous êtes positionné en tant que technicien et qui sont dans l'état à traiter").'" class="nav-link dropdown-toggle pl-lg-3 pr-lg-4 purple" role="button" aria-haspopup="true" aria-expanded="false">
				<i class="fa fa-dollar-sign text-110 mr-lg-2"><!----></i>
				'; if(!$mobile) {echo T_('Facturables');} echo '
				<span id="badge_billable" class="badge badge-sm badge-success radius-round text-80 border-1 brc-white-tp5">'.$billable_counter['counter'].'</span>
			</a>
		</li>
	';
}
...
<li class="nav-item dropdown dropdown-mega">
	<a title="'.T_('Vos tickets résolus').'" class="nav-link dropdown-toggle pl-lg-3 pr-lg-4 purple" href="index.php?page=dashboard&amp;userid='.$_SESSION['user_id'].'&amp;state=3" role="button" aria-haspopup="true" aria-expanded="false">
		...
		<span id="badge_res_tickets" class="badge badge-sm badge-success radius-round text-80 border-1 brc-white-tp5">'.$nbres[0].'</span>
	</a>
</li>
';
?>