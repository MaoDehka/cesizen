...
<form method="post" action="" name="filter" >
	...
	{
		echo '
			<select style="width:160px;" class="form-control form-control-sm d-inline-block" name="company" onchange="submit()" autocomplete="off">
				';
	...
	<select style="width:170px;" class="form-control form-control-sm d-inline-block" name="netbios" id="netbios" onchange="submit()">
	<?php
	$qry=$db->prepare("SELECT `id`,`netbios` FROM `tassets` WHERE `netbios`!='' ORDER BY `netbios`");
	$qry->execute();
	while($row=$qry->fetch()) 
	{
		if($row['netbios'] == $_POST['netbios']) 
		{
			echo '<option value="'.$row["netbios"].'" selected>'.$row['netbios'].'</option>'; 
		} else {
			echo '<option value="'.$row["netbios"].'">'.$row['netbios'].'</option>'; 
		}
	}
	$qry->closeCursor();
	echo '<option '; if ($_POST['netbios']=='%') echo 'selected'; echo' value="%" >'.T_('Tous les équipements').'</option>';										
	?>
	</select> 
