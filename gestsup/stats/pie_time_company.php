<?php
################################################################################
# @Name : pie_time_company.php
# @Description : display Statistics of chart 9 
# @call : /stat.php
# @parameters : 
# @Author : Flox
# @Create : 16/09/2022
...
if($company_counter['0']>0)
{
	...
	//display title
	$libchart=T_('Répartition du temps passé par sociétés');
	$unit=T_('heures');

	//query
	$query1 = "SELECT `tcompany`.`name` AS `company`, ROUND((SUM(`tincidents`.`time`))/60,1) AS `time`
