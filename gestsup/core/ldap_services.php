...
if ($ldapbind) 
{
	...
	for ($i=0; $i < $cnt_group; $i++) 
	{
		...
		$LDAP_group_name_ldap_qry=$data[$i]['distinguishedname'][0];
		$LDAP_group_samaccountname=mb_convert_encoding($data[$i]['samaccountname'][0], 'UTF-8', 'ISO-8859-1');
		...
		if(empty($GS_group['id'])){$GS_group=array(); $GS_group['id']='';}
		if(empty($GS_group['name'])){$GS_group=array(); $GS_group['name']='';}
		if(empty($GS_group['ldap_guid'])){$GS_group=array(); $GS_group['ldap_guid']='';}
		if(empty($GS_group['0'])){$GS_group=array(); $GS_group['0']='';}
		...
		$data2 = array();
		$data2_temp = array();
		$filter2="(&(objectCategory=user)(memberof:1.2.840.113556.1.4.1941:=$LDAP_group_name_ldap_qry))";
		...
		for ($i2=0; $i2 < $cnt_users; $i2++) 
		{
			...
			$GS_user=$qry2->fetch();
			$qry2->closeCursor();
			if(empty($GS_user['id'])) {$GS_user=array(); $GS_user['id']='';}
			if(empty($GS_group['id'])) {$GS_group=array(); $GS_group['id']='';}
			...
			if($GS_user['id'])
			{
				...
				{	
					...
					$qry2->closeCursor();
					if(empty($GS_service['id'])) {$GS_service=array(); $GS_service['id']='';}
					...
					if($user_update) {echo '<i class="fa fa-arrow-circle-up text-success"></i><font class="text-success"> '.T_("Mise à jour de l'utilisateur").': <b>'.$LDAP_login.'</b> (<font size="1">'.T_("Raison: Le guid LDAP du service est identique à celui de GestSup et une différence à été trouvé dans ").' '.$user_update.'</font>)</font><br />';}

				} elseif($_GET['action']=='run') {
					//init var
					if(empty($GS_user['login'])) {$GS_user=array(); $GS_user['login']='';}
					if(empty($GS_user['firstname'])) {$GS_user['firstname']='';}
					if(empty($GS_user['lastname'])) {$GS_user['lastname']='';}
					if(empty($GS_user['mail'])) {$GS_user['mail']='';}
					if(empty($GS_user['phone'])) {$GS_user['phone']='';}
					if(empty($GS_user['address1'])) {$GS_user['address1']='';}
					if(empty($GS_user['zip'])) {$GS_user['zip']='';}
					if(empty($GS_user['city'])) {$GS_user['city']='';}
					if(empty($GS_user['fax'])) {$GS_user['fax']='';}
					if(empty($GS_user['function'])) {$GS_user['function']='';}
					if(empty($GS_user['id'])) {$GS_user['id']='';}
					...
					if($LDAP_login!=$GS_user['login']) 
					{
						...
						$qry2->execute(array('login' => $LDAP_login,'ldap_guid' => $LDAP_user_guid));
					...
					{
						$qry2=$db->prepare("UPDATE `tusers` SET `ldap_sid`=:ldap_sid WHERE `ldap_guid`=:ldap_guid");
						$qry2->execute(array('ldap_sid' => $LDAP_user_sid,'ldap_guid' => $LDAP_user_guid));
					...
					if($user_update) {
						...
						logit('ldap_service','Update user informations from LDAP for user '.$LDAP_login.' ('.$user_update.')','0');
					}
				}
			} else {
				//create user
				...
				}elseif ($_GET['action']=='run')
				{
					...
					$qry2=$db->prepare("
						...
						VALUES (:login,:firstname,:lastname,'2',:mail,:phone,:address1,:zip,:city,:fax,:ldap_sid)
					");
					$qry2->execute(array(
						'login' => $LDAP_login,
					...
					logit('ldap_service','Create user '.$LDAP_login,'0');
				}
			}	
		}
