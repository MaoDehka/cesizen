				...
				<div class="modal-body">
					<form name="form1" method="POST" action="" id="form1">
						...
						<input name="install" type="hidden" value="1">
							'.T_("Si l'opération n'a pas déjà été faites, réaliser une sauvegarde de vos fichiers et de votre base de données").'.
							<br />
							<br />
							<a target="_blank" href="https://doc.gestsup.fr/backup/">'.T_("Plus d'informations").'</a>
					</form>
...
if($step==5)
{
	...
	if($type=='version')
	{
		...
		$matches = glob(__DIR__ ."/../download/tmp/_SQL/update_*.sql"); 
		foreach ($matches as $filename) {
			...
			$subsrc=explode('.',$src);
			if ($subsrc[0]>=$local_version_array[0])
			{
				if ($subsrc[1]>=$local_version_array[1]) 
				{
					if ($subsrc[2]>=$local_version_array[2] || $subsrc[1]>$local_version_array[1]) //fix update
					{
						//import script
	...
	if($type=='patch')
	{
		if($dedicated==1)
		{
			$storefilename='update_'.$local_version_array[0].'.'.$local_version_array[1].'.'.$local_version_array[2].'.'.$local_version_array[3].'_to_'.$local_version_array[0].'.'.$local_version_array[1].'.'.$local_version_array[2].'.'.$next_remote_patch.'.sql';
		} else {
			$storefilename='update_'.$local_version_array[0].'.'.$local_version_array[1].'.'.$local_version_array[2].'_to_'.$local_version_array[0].'.'.$local_version_array[1].'.'.$next_remote_patch.'.sql';
		}
		...
		if($dedicated==1)
		{
			if($vactu==$local_version_array[0].'.'.$local_version_array[1].'.'.$local_version_array[2].'.'.$next_remote_patch) {
			$result=$result.'- '.T_('Modification base de données').': <i class="fa fa-check-circle text-success"></i><br />';
			$step=6;
			} else {
				$result=$result.'- '.T_('Modification base de données').': <i class="fa fa-times-circle text-danger"></i><br />'; 
			...
			if($vactu==$local_version_array[0].'.'.$local_version_array[1].'.'.$next_remote_patch) {
			$result=$result.'- '.T_('Modification base de données').': <i class="fa fa-check-circle text-success"></i><br />';
			$step=6;
			} else {
				$result=$result.'- '.T_('Modification base de données').': <i class="fa fa-times-circle text-danger"></i><br />'; 
