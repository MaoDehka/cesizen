...
if($_GET['key']==$key['server_private_key'])
{
	...
	$qry=$db->prepare("SELECT COUNT(tincidents.id) FROM `tincidents`,`tstates` WHERE `tincidents`.`state`=`tstates`.`id` AND tincidents.disable='0' AND (tstates.meta='1' OR tincidents.state='5')");
	$qry->execute();
	$nbopen=$qry->fetch();
	$qry->closeCursor();

	//query all to do ticket
	$qry=$db->prepare("SELECT COUNT(tincidents.id) FROM `tincidents`,`tstates` WHERE `tincidents`.`state`=`tstates`.`id` AND tincidents.disable='0' AND tstates.meta='1'");
	$qry->execute();
	$nbtodo=$qry->fetch();
	$qry->closeCursor();
	...
	<!DOCTYPE html>
		...
		<body class="m-3">
			...
			<a href="#" class="btn btn-'.$color.' btn-app radius-4 mr-2 mr-2">
				'.$new.'<br />'.$ticket.'
				<div class="mt-4"></div>
				<i class="fa fa-ticket text-200"></i>
				<div class="mt-4"></div>
			...
			{
				echo '
				<a href="#" class="btn btn-warning btn-app radius-4 mr-2 text-white">
	...
	echo '<br /><br /><div class="text-center" style="color:red;">Cette page a été déplacée, utiliser le lien présent dans Administration > Paramètres > Général "Écran de supervision"</div>';
}
